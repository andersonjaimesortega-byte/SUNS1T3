    ...
    <div className="space-y-8 py-4 animate-in fade-in duration-500">
      <section className="bg-[var(--color-brand-blue)]/5 border border-[var(--color-brand-blue)]/10 p-8 rounded-[32px] relative overflow-hidden group">
        ...
        <div className="relative z-10">
          ...
          >
            <PlusCircle className="w-5 h-5 font-bold" /> Generar Reporte
          </button>
        </div>
      </section>
      ...
      {recentReports.length > 0 && (
        <section className="space-y-4 animate-in slide-in-from-bottom-4 duration-700">
          <div className="flex items-center justify-between px-2">
            ...
            <button onClick={() => navigate('/history')} className="text-[10px] font-bold text-[var(--color-brand-blue)] uppercase tracking-tight hover:underline">Auditar Todo</button>
          </div>
...
const SuccessView = ({ lastReport }) => {
  const { user } = useAuth();
  const navigate = useNavigate();

  // Si no hay reporte, vuelve al dashboard
  if (!lastReport) {
    navigate('/');
    return null; // o un pequeño text de cargando si hubiera re-render extraño.
  }
    ...
    try {
      ...
      if (navigator.share) {
        await navigator.share({
          ...
          text: `Comparto informe: ${reportData.minigranjaId}`
        });
      } else {
